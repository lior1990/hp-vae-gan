...
def netG_forward_without_encoder(netG, z, noise_amps, mode="rec"):
    vae_out = torch.tanh(netG.decoder(z))
    return netG.refinement_layers(0, vae_out, noise_amps, mode)


def optimize_encoder(opt, netG, encoder):
    ...
    for iteration in epoch_iterator:
        ...
        z = encoder(real)
        generated = netG_forward_without_encoder(netG, z, opt.Noise_Amps)

        rec_loss = opt.rec_weight * opt.rec_loss(generated, real)

        encoder.zero_grad()
        rec_loss.backward()
        torch.nn.utils.clip_grad_norm_(encoder.parameters(), opt.grad_clip)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--netG', help='path to netG (to continue training)', required=True)
    ...
    latent_spatial_dimensions = utils.get_scales_by_index(0, opt.scale_factor, opt.stop_scale, opt.img_size)
    latent_spatial_dimensions = [int(latent_spatial_dimensions * opt.ar), latent_spatial_dimensions]
    encoder = Encode2DAE(opt, opt.latent_dim, latent_spatial_dimensions, num_blocks=2*opt.enc_blocks)
    encoder.to(opt.device)

    optimize_encoder(opt, netG, encoder)
